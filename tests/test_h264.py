...
def decode_vaapi_ffmpeg(stream_name, va_stream_name, output_name):
    ret = False 
    ...
    cmdstr = FFMPEG_FILE_PATH + " -loglevel verbose -hwaccel vaapi -hwaccel_device /dev/dri/renderD128 -i " + stream_name + " " + output_name + " -y"
...
def decode_cnm_ref_c(stream_name, va_stream_name, output_name):
    ret = False
    try:
...
def compare_output(golden, compare):
    ret = False
    try:
...
def test_h264_streams(stream_name):
    ...
    if stream_name.find("AIR_320x240_264") == -1:
       pass
        ...
        decode_vaapi_ffmpeg(stream_name, va_stream_name, output_file_ffmpeg)
        decode_cnm_ref_c(stream_name, va_stream_name, output_file_cmodel)
        ret = compare_output(output_file_ffmpeg, output_file_cmodel)
...
#         decode_vaapi_ffmpeg(stream_name, va_stream_name, output_file_ffmpeg)
#         decode_cnm_ref_c(stream_name, va_stream_name, output_file_cmodel)
#         if compare_output(output_file_ffmpeg, output_file_cmodel) == True:
#             print("compare file!!!")
#         # assert compare_output(output_file_ffmpeg, output_file_cmodel) == True
